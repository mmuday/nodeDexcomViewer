 ...
 * Note most of this isn't used; need to remove most of this stuff... but for now... ;) mmuday
...
function readENV(varName, defaultValue) {
    ...
    var value = process.env['CUSTOMCONNSTR_' + varName] || process.env['CUSTOMCONNSTR_' + varName.toLowerCase()] || process.env[varName] || process.env[varName.toLowerCase()];
 ...
 firstFetchCount: readENV('firstFetchCount', 3)
  ...
  do_everything(meta, function (err, glucose) {
    if (glucose) {
      ...
      //console.log('Entries', entries);
      var entry = entries[0];
      var time = moment(entry.date).fromNow();
      console.log(entry.sgv,entry.direction,time);
    }
  });
